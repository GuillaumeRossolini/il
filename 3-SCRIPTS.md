...
You can choose to install them from any package manager of your choice, or you can [build them from source](./scripts/BUILDS.sh). We won’t go into the details of installing each here, but you can find [here](./scripts/BUILDS.sh) a script that should build all of it on a vanilla Debian install (be advised: don't do this on a production system, but only on a fresh environment for testing purposes).
